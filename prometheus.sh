...
function Load() {
docker load -i ./node.tar &> /dev/null && docker load -i ./prometheus.tar.gz &> /dev/null && docker load -i ./alertmanager.tar &> /dev/null && docker load -i ./grafana.tar.gz &> /dev/null
  if [ $? -eq 0 ];then
